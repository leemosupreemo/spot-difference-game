...
Builds, packages, and uploads Diff Hunter iOS app to Firebase App Distribution with incremented build version.
...
echo "🏗️  3. Archiving Xcode app project (Build #${BUILD_NUMBER}, Team: '$DEVELOPMENT_TEAM')..."
archive_cmd=(
  xcodebuild archive
...
echo "🚀 5. Uploading and distributing Build #${BUILD_NUMBER} to Firebase App Distribution..."
...
if CI=1 "${dist_cmd[@]}" < /dev/null; then
  echo "----------------------------------------------------"
  echo "🎉 FIREBASE APP DISTRIBUTION NEW RELEASE SUCCEEDED!"
  echo "📱 New Build #${BUILD_NUMBER} uploaded to Firebase App Distribution."
